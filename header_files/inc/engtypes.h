...
// CVADHELP-24945: support long FQDN for DNS query
typedef CHAR FQDN[FQDN_LENGTH + 1];
...
// CVADHELP-24945: support long FQDN for DNS query
typedef WCHAR FQDNW[FQDN_LENGTH + 1];
...
typedef struct _WFEOPEN {
   ...
   ULONG_PTR          reserved;          /* Reserved - must be 0 */
   /* below is for API Version 6 and above */
   LPCWSTR             pszClientnameW;    /* WIN (optional) obsoletes pszClientname */
   LPCWSTR             pszTitleW;         /* WIN (optional) obsoletes pszTitle */
...
//#endif
	...
	WFEAppProtectionEnabled,             /* Query */
	WFECasInfo,
	WFECLStatus,                             // set
	WFECLSessionInfo,
    WFEMarshallVdInfo,                       // set or query in vc thread
    WFETransactionId,             /* Set & Query */
	WFEScreenProtectionEnabled,			//Query
	WFEStoreDetailForInAppLaunch,	 //Query
	WFEStoreDetailForWebLaunch,			//Query
    WFEToolbarCommWC,
    WFEAADSSOnStatus,
    WFEAADSSOnAuthInfo,
    WFEGetSessionGUID,
	WFEConnectionMetrics,
	WFESessionId,
    WFEEuemLogonInfo,	          /* Set */
} WFEINFOCLASS;
...
typedef struct _WFECLSTATUS
{
    ...
    DWORD   cbGCTLength;
    DWORD   cbRCTLength;
    DWORD   cbVdaPublicKeyLength;
    DWORD   cbVdaAuthInfoLength;
    BYTE    Data[1];
}
WFECLSTATUS, *PWFECLSTATUS;
...
// Required because Eng_Defines.h does explicitly defines _ENGTYPE_H_
#ifndef _ENGTYPES2_H_
#define _ENGTYPES2_H_

#ifdef PLATFORM_PRAGMAS
#pragma ENG_PACKING_ENABLE
#endif

// Data block to indicate if the Host is Azure AD SSOn capable and request the corresponding
// authentication information (Access Token, Client Nonce...). Exchanged from Engine to CCM.
typedef struct _WFEAADSSONSTATUS
{
    HANDLE hWFW;
    BOOL   fRequestAuthInfo; // Flag to request Azure AD SSOn info from SSP.
    BOOL   fSendHostInfo; // Flag to inform wfcrun32 that it needs to send Host info.
    BOOL   fHostCapable; // Flag to inform if VDA supports Azure AD SSOn.
    BOOL   fLaunchSuccess; // Flag to inform wfcrun32 that launch was successful.
    BOOL   fLaunchFailed; // Flag to inform wfcrun32 that launch failed.
    BOOL   fSentErrorInfo; // Flag to inform wfcrun32 that it needs to send error info.
    UINT   errorCode; // Error code to be send back if there was any issue during launch.
    INT    azureDeviceIdOffset; // Offset to Azure Device Id null-terminated string.
    INT    azureDeviceIdSize; // Count include null-terminating character.
    CHAR   azureDeviceId[1]; // Multistring (double null-terminated).
} WFEAADSSONSTATUS, * PWFEAADSSONSTATUS;

// Data block to exchange the AAD SSOn authentication information between CCM and Engine.
// If a field is not available yet, both offset and count are zero.
typedef struct _WFEAADSSONAUTHINFO
{
    ULONG CorrelationIdOffset; // Offset to CorrelationId null-terminated string.
    ULONG CorrelationIdCharCount; // Count includes null-terminating character.
    ULONG DeviceIdOffset; // Offset to DeviceId null-terminated string.
    ULONG DeviceIdCharCount; // Count Include null-terminating character.
    ULONG AccessTokenOffset; // Offset to Access Token null-terminated string.
    ULONG AccessTokenCharCount; // Count include null-terminating character.
    ULONG ClientNonceOffset; // Offset to Client Nonce null-terminated string.
    ULONG ClientNonceCharCount; // Count include null-terminating character.
    CHAR Data[1]; // Multistring (double null-terminated).
} WFEAADSSONAUTHINFO, * PWFEAADSSONAUTHINFO;
